...
Exports complete repository history including:
- Git commits with full details
- GitHub PR reviews and approvals
- PR comments and discussions
- Issue references and tracking

Modular design - enable only the data you need.
...
Setup:
    export GITHUB_TOKEN='your_token_here'  # For GitHub API features
...
# GitHub API Configuration (optional - set these for GitHub-specific data)
GITHUB_TOKEN = os.getenv('GITHUB_TOKEN', '')  # Set in environment or paste here
GITHUB_API = "https://api.github.com"

# Module Selection - Set to True/False for what you want to export
EXPORT_MODULES = {
    ...
    'commit_stats': True,        # File changes and diff stats per commit
    'commit_files': True,        # Detailed file changes
    'commit_diffs': False,       # Full diff content (can be very large)
    'pr_data': True,             # Pull request information (requires GitHub API)
    'pr_reviews': True,          # PR reviews and approvals (requires GitHub API)
    'pr_comments': True,         # PR review comments (requires GitHub API)
    'issue_references': True,    # Issues mentioned in commits (requires GitHub API)
}

# Commit limit (None for all commits, or set a number like 100)
COMMIT_LIMIT = None
...
def main():
    print("🔍 Extracting Git & GitHub History...\n")
    ...
    # Collect all data based on module selection
    data = {}
    
    # Repository info
    if EXPORT_MODULES['repo_info']:
        print("📊 Gathering repository info...")
        data['repo_info'] = get_repo_info()
    
    # Contributors
    if EXPORT_MODULES['contributors']:
        print("👥 Gathering contributors...")
        data['contributors'] = get_contributors()
    
    # Branches
    if EXPORT_MODULES['branches']:
        print("🌿 Gathering branches...")
        data['branches'] = get_branches()
    
    # Tags
    if EXPORT_MODULES['tags']:
        print("🏷️  Gathering tags...")
        data['tags'] = get_tags()
    
    # Commits
    if EXPORT_MODULES['commits']:
        print(f"📝 Gathering commits{f' (limit: {COMMIT_LIMIT})' if COMMIT_LIMIT else ' (all)'}...")
        commit_hashes = get_all_commits(limit=COMMIT_LIMIT)
        commits = []
        
        for i, commit_hash in enumerate(commit_hashes, 1):
            if commit_hash:
                print(f"   Processing commit {i}/{len(commit_hashes)}...", end='\r')
                commit_data = get_commit_details(
                    commit_hash,
                    include_stats=EXPORT_MODULES['commit_stats'],
                    include_files=EXPORT_MODULES['commit_files'],
                    include_diff=EXPORT_MODULES['commit_diffs']
                )
    ...
    # GitHub API data (if token is available)
    ...
    print(f"   ✅ Saved: {OUTPUT_HUMAN} ({len(human_content):,} chars)")
    
    print("🤖 Generating AI-optimized format...")
    ai_content = generate_ai_optimized(data)
    with open(OUTPUT_AI, 'w', encoding='utf-8') as f:
        f.write(ai_content)
    print(f"   ✅ Saved: {OUTPUT_AI} ({len(ai_content):,} chars)")
    
    print("\n✨ Complete! Generated files:")
    print(f"   • {OUTPUT_HUMAN} - Human-readable with navigation")
    print(f"   • {OUTPUT_AI} - AI-optimized flat structure")
    
    if not GITHUB_TOKEN:
        print("\n💡 Tip: Set GITHUB_TOKEN environment variable for PR data:")
        print("   export GITHUB_TOKEN='your_token_here'")
        print("   or edit GITHUB_TOKEN in the script")
